...
func add_new_transaction(transaction_p):
	...
	temp_class.total_gains_m = String(float(temp_class.total_sold_m) - float(temp_class.total_paid_m));
	if float(temp_class.total_paid_m) != 0:
		temp_class.percent_gains_m = String((float(temp_class.total_gains_m) / float(temp_class.total_paid_m)) * 100);
